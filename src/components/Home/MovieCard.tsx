...
function MovieCard({ movieData }: MovieCardProps) {
  ...
  return (
    <Link to={'/details/' + movieData.id}>
      ...
      >
        <div
          className={`my-3 border-2 border-zinc-800 rounded-lg overflow-hidden ${movieData.id === hover ? "scale-[102%] text-yellow-500" : ""} duration-200 transition-transform`}
        >
          ...
          <div className="bg-[#222] p-2 ">
            ...
            <div className="text-zinc-300 text-[15px] mt-2">
              <h1 className="text-[15px] text-zinc-300">
                Rating: {String(movieData.vote_average).substring(0, 3)}
              </h1>
              <h1 className="">Language: {movieData.original_language}</h1>
              <h1 className="">Release: {movieData.release_date}</h1>
            </div>
          </div>
        </div>
      </div>
